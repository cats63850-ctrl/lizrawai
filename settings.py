...
def _count_matching_sync(
    storage, guild_id: int, author_id: int | None, pattern: str | None
) -> int:
    """Сколько сообщений попадёт под удаление.

    Условия повторяют Storage._wipe_sync один в один: если они разойдутся,
    предпросмотр начнёт врать, а это худшее, что может сделать
    ...
    row = storage.conn.execute(
        f"SELECT COUNT(*) AS n FROM messages WHERE {' AND '.join(where)}", params
    ).fetchone()
    return row["n"]
...
class Settings(commands.Cog, name="Настройки"):
    ...
    @commands.hybrid_command(
        name="forget",
        aliases=["забудь"],
        description="Удалить из корпуса сообщения человека или по тексту",
    )
    @app_commands.describe(
        user="Чьи сообщения удалить",
        text="Текст для поиска — или ID человека, если он вышел с сервера",
    )
    @commands.guild_only()
    @commands.has_permissions(manage_guild=True)
    async def forget(
        ...
        *,
        ...
        if user is not None:
            author_id = user.id
            scope = f"все сообщения {user.display_name}"
        elif text and text.strip().isdigit():
            # По ID — единственный способ вычистить того, кто вышел
            # с сервера или чей аккаунт заблокировал сам Discord.
            author_id = int(text.strip())
            scope = f"все сообщения человека с ID {author_id}"
        elif text:
            pattern = text.strip()
            scope = f"сообщения со словом «{pattern}»"
        else:
            await ctx.send(
                "Кого или что забыть?\n"
                f"`{prefix}forget @кто-то` — все сообщения человека\n"
                f"`{prefix}forget 123456789` — по ID, если человек вышел\n"
                f"`{prefix}forget слово` — все сообщения с этим словом"
            )
            return

        storage = self.bot.storage
        found = await storage._run(
            _count_matching_sync, storage, ctx.guild.id, author_id, pattern
        )
        if not found:
            await ctx.send("Под это ничего не подходит, корпус не тронут.")
            return

        view = ConfirmView(ctx.author.id)
        message = await ctx.send(
            f"Найдено сообщений: **{found}**. Удалить {scope}? Это необратимо.",
            view=view,
        )
        await view.wait()
        ...
        removed = await storage.wipe(
            ctx.guild.id, author_id=author_id, pattern=pattern
        )
        self.bot.drop_model(ctx.guild.id)
        await message.edit(
            content=f"Удалено сообщений: {removed}. Модель пересоберётся.",
            view=view,
        )
